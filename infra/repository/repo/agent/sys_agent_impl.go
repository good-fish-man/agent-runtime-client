...
func (r *SysAgentRepo) Delete(ctx context.Context, en *entity.SysAgent) error {
	...
	return r.data.DB(ctx).Model(&po.SysAgent{}).Where("ulid = ?", en.Ulid).Updates(patch).Error
}
...
func (r *SysAgentRepo) Update(ctx context.Context, en *entity.SysAgent) error {
	...
	return db.Updates(map[string]any{"embedding_model": en.EmbeddingModel, "image_model": en.ImageModel}).Error
}
