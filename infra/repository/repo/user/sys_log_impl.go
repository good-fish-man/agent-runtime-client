...
// SysLogRepo is the gorm-backed implementation of ISysLogRepo.
type SysLogRepo struct {
	data *data.Data
}

// NewSysLogRepo constructs the repository with the shared data handle.
func NewSysLogRepo(d *data.Data) *SysLogRepo {
	return &SysLogRepo{data: d}
}
...
func (r *SysLogRepo) Delete(ctx context.Context, en *entity.SysLog) error {
	...
	return log.WrapError(r.data.DB(ctx).Model(&po.SysLog{}).Where("ulid = ?", en.Ulid).Updates(patch).Error, "Repository")
}
