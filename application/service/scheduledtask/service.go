...
func (s *Service) Create(ctx context.Context, req CreateRequest) (*po.ScheduledTask, error) {
	...
	if req.MisfirePolicy == "" {
		req.MisfirePolicy = orchestrationv2.MisfireOnce
	}
	if req.RetryMax == 0 {
		req.RetryMax = 3
	}
	...
	if req.ApprovalMode == "" {
		req.ApprovalMode = orchestrationv2.ApprovalNone
	}
	notify := true
	if req.Notify != nil {
		notify = *req.Notify
	...
	schedule := orchestrationv2.Schedule{Schema: orchestrationv2.Schema, ScheduleID: item.Ulid, OwnerID: item.UserID, Name: item.Name, Cron: item.CronExpr, Timezone: item.Timezone, MisfirePolicy: item.MisfirePolicy, Retry: orchestrationv2.RetryPolicy{MaxAttempts: item.RetryMax, BackoffMS: item.RetryBackoffMS}, MaxConcurrency: item.MaxConcurrency, RiskLevel: item.RiskLevel, ApprovalMode: item.ApprovalMode, PreauthorizationID: item.PreauthorizationID, Notify: item.Notify, Status: orchestrationv2.ScheduleActive, Revision: 1, CreatedAt: time.Now().UTC(), UpdatedAt: time.Now().UTC()}
...
func (s *Service) Start(parent context.Context) {
	if s == nil || s.data == nil || s.orchestration == nil {
		return
	}
	s.lifecycleMu.Lock()
...
func (s *Service) loop(ctx context.Context) {
	...
	log.Infof(ctx, "scheduled task scanner started; interval=%s", interval)
...
func (s *Service) tick(ctx context.Context) {
	...
	if err := s.data.DB(ctx).Where("status = ? AND deleted_at = 0", po.StatusActive).Find(&tasks).Error; err != nil {
		log.Errorf(ctx, "scheduled task scan failed: %v", err)
		return
	}
	for index := range tasks {
		...
		if s.activeTriggerCount(ctx, task) >= task.MaxConcurrency {
			log.Warnf(ctx, "scheduled task durable concurrency limit reached; task=%s limit=%d", task.Ulid, task.MaxConcurrency)
			continue
		}
		if !s.acquire(task.Ulid, task.MaxConcurrency) {
			log.Warnf(ctx, "scheduled task per-task concurrency limit reached; task=%s limit=%d", task.Ulid, task.MaxConcurrency)
			continue
		}
		select {
		case s.sem <- struct{}{}:
		default:
			s.release(task.Ulid)
			log.Warnf(ctx, "scheduled task global concurrency limit reached; task=%s", task.Ulid)
			continue
		}
...
func (s *Service) execute(task po.ScheduledTask, slot string) {
	...
	requireApproval := task.ApprovalMode == orchestrationv2.ApprovalBeforeRun
	approvalID := ""
	if requireApproval {
		approvalID = ulid.New()
	}
	state, err := s.orchestration.CreateScheduledGoal(ctx, task.UserID, orchestrationsvc.CreateScheduledGoalRequest{
		CreateGoalRequest: orchestrationsvc.CreateGoalRequest{
			...
			SuccessCriteria: []orchestrationv2.SuccessCriterion{{Description: scheduledCriterion(task), Required: true}},
			ApprovalPolicy:  orchestrationv2.ApprovalPolicy{RequireBeforeRisks: []string{task.RiskLevel}, PreauthorizationID: task.PreauthorizationID},
		},
		ScheduleID: task.Ulid, Slot: slot, ScheduledAt: started,
		Retry:  orchestrationv2.RetryPolicy{MaxAttempts: task.RetryMax, BackoffMS: task.RetryBackoffMS},
		Notify: task.Notify, RequireApproval: requireApproval, PendingApprovalID: approvalID,
		Task: orchestrationsvc.PlanTaskRequest{TaskID: "scheduled-" + ulid.New(), Depth: 1, Specialist: orchestrationv2.SpecialistResearch, Objective: prompt, RequiredCapabilities: []string{"internet.search", "internet.fetch"}},
	})
	if err != nil {
		message := truncate(err.Error(), resultLimit)
		log.Errorw(ctx, "scheduled trigger could not create durable goal", "schedule_id", task.Ulid, "slot", slot, "error_chain", log.FormatError(err))
	...
	if requireApproval {
		...
		if createErr := s.data.DB(ctx).Create(&approval).Error; createErr != nil {
			log.Errorw(ctx, "create scheduled pre-execution approval failed", "trigger_id", state.Trigger.TriggerID, "error_chain", log.FormatError(createErr))
			if _, cancelErr := s.orchestration.RejectApproval(ctx, task.UserID, state.State.Goal.GoalID, approvalID, "pre-execution approval could not be persisted", 0); cancelErr != nil {
				log.Errorw(ctx, "cancel schedule after approval persistence failure failed", "trigger_id", state.Trigger.TriggerID, "error_chain", log.FormatError(cancelErr))
			}
		}
	}
}
...
func (s *Service) activeTriggerCount(ctx context.Context, task po.ScheduledTask) int {
	...
	if err != nil {
		log.Errorw(ctx, "list active schedule triggers failed", "schedule_id", task.Ulid, "error_chain", log.FormatError(err))
		return task.MaxConcurrency
	}
	count := 0
	for _, trigger := range triggers {
		switch trigger.Status {
		case orchestrationv2.ScheduleTriggerQueued, orchestrationv2.ScheduleTriggerRunning, orchestrationv2.ScheduleTriggerWaitingUser, orchestrationv2.ScheduleTriggerWaitingDevice:
			count++
		}
	}
	return count
...
func (s *Service) reconcileTerminalTriggers(ctx context.Context, task po.ScheduledTask) {
	...
	if err != nil {
		log.Errorw(ctx, "list schedule triggers for reconciliation failed", "schedule_id", task.Ulid, "error_chain", log.FormatError(err))
		return
	}
	for _, trigger := range triggers {
...
func (s *Service) reconcileTerminalTrigger(ctx context.Context, task po.ScheduledTask, trigger orchestrationv2.ScheduleTrigger) {
	finished := trigger.FinishedAt
	if finished.IsZero() {
		finished = time.Now().UTC()
	}
	status := jobpo.JobStatusSuccess
	if trigger.Status == orchestrationv2.ScheduleTriggerFailed || trigger.Status == orchestrationv2.ScheduleTriggerCancelled {
	...
	if err := s.data.DB(ctx).Model(&jobpo.JobExecutionPO{}).Where("ulid = ?", job.Ulid).Count(&existing).Error; err == nil && existing == 0 {
		if err := s.data.DB(ctx).Create(&job).Error; err != nil {
			log.Errorw(ctx, "persist reconciled schedule execution failed", "trigger_id", trigger.TriggerID, "error_chain", log.FormatError(err))
			return
		}
	}
	...
	if err := s.data.DB(ctx).Model(&po.ScheduledTask{}).Where("ulid = ?", task.Ulid).Updates(updates).Error; err != nil {
		log.Errorw(ctx, "update reconciled schedule status failed", "trigger_id", trigger.TriggerID, "error_chain", log.FormatError(err))
		return
	}
	notificationStatus, notificationError := orchestrationv2.NotificationDisabled, ""
	if trigger.Notify {
		notificationStatus = orchestrationv2.NotificationSent
		if strings.TrimSpace(task.SessionID) == "" {
			notificationStatus, notificationError = orchestrationv2.NotificationFailed, "schedule has no conversation session for notification delivery"
		} else {
			...
			if err := s.data.DB(ctx).Create(&message).Error; err != nil {
				...
				if countErr := s.data.DB(ctx).Model(&chatpo.ChatMessage{}).Where("ulid = ?", message.Ulid).Count(&count).Error; countErr != nil || count == 0 {
					notificationStatus, notificationError = orchestrationv2.NotificationFailed, err.Error()
				}
			}
		}
	}
	if err := s.orchestration.MarkScheduleTriggerReconciled(ctx, task.UserID, trigger, notificationStatus, notificationError); err != nil {
		log.Errorw(ctx, "mark schedule trigger reconciled failed", "trigger_id", trigger.TriggerID, "error_chain", log.FormatError(err))
	}
...
func terminalTrigger(status string) bool {
	return status == orchestrationv2.ScheduleTriggerCompleted || status == orchestrationv2.ScheduleTriggerFailed || status == orchestrationv2.ScheduleTriggerCancelled
}
...
func dueSlot(task po.ScheduledTask, now time.Time) (string, bool) {
	...
	if task.MisfirePolicy != orchestrationv2.MisfireOnce || task.LastRunAt <= 0 {
		return "", false
	}
